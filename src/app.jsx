...
function App() {
  ...
  useEffectA(() => { window.scrollTo(0, 0); }, [screen, selectedPkg]);
  ...
  const handleNavigate = (s, cat, temp, dest, search, promo) => {
    ...
    else if (s === 'listado' && !temp && !dest && !promo) setInitialCat('todos');
    if (temp) setInitialTemp(temp);
    else if (s === 'listado' && !cat && !dest && !promo) setInitialTemp('todas');
    if (dest) setInitialDest(dest);
    else if (s === 'listado') setInitialDest('todos');
  ...
  return (
    <div className="nt-app" data-screen-label={screen}>
      <TopStrip/>
      <Header onNavigate={handleNavigate} onWhatsapp={() => handleWhatsapp(null)} current={screen}/>
      ...
      {screen === 'detalle' && selectedPkg && (
        <DetalleScreen
          pkg={selectedPkg}
          onBack={() => setScreen('listado')}
          onWhatsapp={(inq) => handleWhatsapp(inq || { pkg: selectedPkg })}
        />
      )}

      <Footer onNavigate={handleNavigate}/>
      <Fab onClick={() => handleWhatsapp(null)}/>
